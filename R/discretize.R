...
idx <- do.call(rbind, lapply(p$object$topology_, \(xx) matrix(unlist(xx[c(".vx0", ".vx1")], use.names = FALSE), ncol = 2)))
 ...
 edges <- as.matrix(dplyr::arrange(tibble::as_tibble(edges), .x0, dplyr::desc(.y0)))
  ...
  for (irow in seq_along(row_ys)) {
    ...
    if (any(active)) {
      ...
      #active_edges <- as.matrix(dplyr::arrange(tibble::as_tibble(active_edges), .x0, dplyr::desc(.y0)))



      (ycount <- ceiling((active_edges[, ".y0"] - row_ys[irow]) / resy))
      s = (active_edges[,".y1"] - active_edges[,".y0"]) /
                   (active_edges[,".x1"] - active_edges[,".x0"])
      (x0 <- active_edges[, ".x0"])
      s <- 1/s
      s[!is.finite(s)] <- 0
      (xx <- x0 + -s * resy * ycount)
